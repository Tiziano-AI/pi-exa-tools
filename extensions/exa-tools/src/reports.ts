...
export function renderStatusReport(projectRoot: string, authState: ExaAuthState, config: ExaOperatorConfig, health: HealthStatus): string {
	...
	const lines = [
		...
		"- Workflow: broad neutral discovery first for unknown/current topics; includeDomains only for known or discovered authority domains",
		"- Currentness: include the time horizon in queries and compare visible dates or versions before making current claims",
